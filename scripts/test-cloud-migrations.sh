...
cleanup() {
  if [[ "$(docker inspect --format '{{index .Config.Labels "kordi.task"}}' "$container_id" 2>/dev/null || true)" == synthetic-migration-check ]]; then
...
for test_name in "${tests[@]}"; do
  ...
  docker exec "$container_id" createdb -U postgres "$database"
  KORDI_MIGRATION_TEST_DATABASE_URL="postgresql://postgres@127.0.0.1:${port}/${database}" \
    cargo test -p kordi-cloud-server --lib "pg::pool::upgrade_tests::${test_name}" -- --ignored --exact
done
docker exec "$container_id" createdb -U postgres kordi_migration_test_runtime
...
DATABASE_URL="postgresql://postgres@127.0.0.1:${port}/kordi_migration_test_runtime" \
