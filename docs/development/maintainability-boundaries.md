...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | Shared-agent target resolution and claim authorization are isolated in `runs/authorization.rs`. Continue with envelope, prompt-history, claim/lease, and completion boundaries one responsibility at a time. |
