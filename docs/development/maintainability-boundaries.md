...
| Desktop Rust chat | `app/desktop/src-tauri/src/chat.rs` | Attachment/artifact helpers, live-turn state helpers, Bridge-agent runner logic, and local model-option enrichment have been extracted. Next safe slice: move session action helpers behind existing Tauri command names. |
