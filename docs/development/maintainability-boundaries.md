...
| Local app server | `app/server/src/lib.rs` | The regression suite is isolated in `app/server/src/tests.rs`. Production extraction is proceeding by stable HTTP, session projection, Bridge health, turn execution, and configuration boundaries so the root can retain only server composition. |
