...
| Canonical sessions | `app/desktop/src-tauri/src/canonical_sessions.rs` | Canonical session tests are partitioned by scenario, and identity/session request helpers plus group participant/admin mutations have been extracted. Next safe slice: split message persistence/reconciliation helpers to get the root below the 1,000-line scan threshold. |
