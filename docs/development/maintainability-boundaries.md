...
| Desktop Rust chat | `app/desktop/src-tauri/src/chat.rs` | Attachment/artifact helpers, live-turn state helpers, Bridge-agent runner logic, Bridge outreach helpers, local model-option enrichment, and session action helpers have been extracted. Next safe slice: move canonical sync/projection helpers or split root tests behind existing Tauri command names. |
