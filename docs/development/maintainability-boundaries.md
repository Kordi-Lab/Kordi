...
| Local app server | `app/server/src/lib.rs` | Regression tests, turn execution, configuration, Bridge health, protocol mapping, and typed session read/fork projection are isolated behind focused modules. Production extraction is proceeding with the HTTP boundary so the root can retain only server composition. |
