...
| Desktop Rust chat | `app/desktop/src-tauri/src/chat.rs` | Stable Tauri DTOs, process-local blank-draft lifecycle, attachment/artifact helpers, live-turn state helpers, agent runner/preparation logic, canonical sync projection, message route helpers, local model-option/readiness helpers, session action helpers, and remaining root tests have been extracted. Persisted-session loading remains the next focused boundary. |
