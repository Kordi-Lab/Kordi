...
| Local app server | `app/server/src/lib.rs` | Regression tests, process-based turn execution, environment/filesystem configuration, and Bridge health transport/projection are isolated behind focused modules. Production extraction is proceeding by stable HTTP, session projection, and protocol boundaries so the root can retain only server composition. |
