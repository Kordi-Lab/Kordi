...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | Shared-agent authorization, Cloud message envelopes, and bounded prompt/history assembly are isolated in focused child modules. Continue with claim/lease and completion boundaries one responsibility at a time. |
