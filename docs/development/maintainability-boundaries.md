...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | Authorization, envelope codecs/lookups, prompt assembly, claims, and runner leasing/projection are isolated in focused child modules. Continue with delivery and completion boundaries one responsibility at a time. |
