...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | Authorization, envelopes, prompts, claims, leases, and durable response delivery are isolated in focused child modules. Continue with completion state transitions as the final production boundary. |
