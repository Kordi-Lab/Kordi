...
| Authentication storage | `agent/crates/cli/src/login/store.rs` | The serialized model/redacted debug contract, generic profile-selection/update policy, legacy migration, and regression suite are isolated. Persistence and GitHub Copilot configuration are being separated behind stable auth-store contracts. |
