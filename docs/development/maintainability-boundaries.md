...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | Authorization, envelope codecs/lookups, bounded prompt assembly, and idempotent run claiming are isolated in focused child modules. Continue with lease and completion boundaries one responsibility at a time. |
