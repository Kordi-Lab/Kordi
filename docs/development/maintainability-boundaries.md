...
| Canonical sessions | `app/desktop/src-tauri/src/canonical_sessions.rs` | Canonical session tests are partitioned by scenario, and identity SQLite persistence, identity/session request helpers, group participant/admin mutations, message lookup/dedup, and title reconciliation policy have been extracted. The 1,657-line production root remains tracked by #838; next split sessions, messages, delegated exchanges, and identity migration. |
