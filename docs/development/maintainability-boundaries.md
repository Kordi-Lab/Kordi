...
| Authentication storage | `agent/crates/cli/src/login/store.rs` | The serialized model/redacted debug contract, generic profile-selection/update policy, legacy migration, atomic persistence, and regression suite are isolated. GitHub Copilot configuration is being separated behind stable auth-store contracts. |
