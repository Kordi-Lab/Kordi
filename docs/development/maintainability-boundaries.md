...
| Authentication storage | `agent/crates/cli/src/login/store.rs` | The serialized model/redacted debug contract and regression suite are isolated. Generic profile selection, migration, persistence, and GitHub Copilot configuration are being separated behind stable auth-store contracts. |
