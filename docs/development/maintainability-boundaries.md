...
| Local app server | `app/server/src/lib.rs` | Regression tests, process-based turn execution, configuration, Bridge health, and protocol/header/timeline projection are isolated behind focused modules. Production extraction is proceeding by stable HTTP and session projection boundaries so the root can retain only server composition. |
