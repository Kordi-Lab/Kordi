...
| Desktop Agent Builder | `app/desktop/src-tauri/src/chat/agent_builder.rs` | Models, durable storage, workspace validation/fingerprinting, atomic workspace lifecycle, and runtime setup/smoke execution are isolated behind focused modules. The 646-line composition/command root is below the 1,000-line scan threshold; publishing orchestration remains the named follow-up boundary. |
