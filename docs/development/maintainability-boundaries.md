...
| Desktop Rust chat | `app/desktop/src-tauri/src/chat.rs` | Stable Tauri DTOs, attachment/artifact helpers, live-turn state helpers, agent runner/preparation logic, canonical sync projection, message route helpers, local model-option/readiness helpers, session action helpers, and remaining root tests have been extracted. Transient draft activation remains the next focused boundary. |
