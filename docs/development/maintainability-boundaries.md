...
| Canonical sessions | `app/desktop/src-tauri/src/canonical_sessions.rs` | Canonical session tests are partitioned by scenario; identity persistence and migration, identity/session request helpers, group participant/admin mutations, message lookup/dedup, and title reconciliation policy have been extracted. The 1,042-line composition root remains tracked by #838; its next focused persistence extraction brings it below the scan threshold. |
