...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | Shared-agent authorization and Cloud message envelope codecs/lookups are isolated in focused child modules. Continue with prompt-history, claim/lease, and completion boundaries one responsibility at a time. |
