...
| Authentication storage | `agent/crates/cli/src/login/store.rs` | The serialized model/redacted debug contract, generic profile-selection/update policy, and regression suite are isolated. Migration, persistence, and GitHub Copilot configuration are being separated behind stable auth-store contracts. |
