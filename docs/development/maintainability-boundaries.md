...
| Desktop Rust chat | `app/desktop/src-tauri/src/chat.rs` | Stable Tauri DTOs, draft/session lifecycle, attachments/artifacts, live-turn state, runner preparation, canonical sync, routing, model readiness, session actions, and tests are isolated. The 685-line root is a stable Tauri command facade; the 270-line `chat/message_execution.rs` owns background turn orchestration through a typed request. |
| Desktop Agent Builder | `app/desktop/src-tauri/src/chat/agent_builder.rs` | Models, durable storage, workspace validation/fingerprinting, atomic workspace lifecycle, runtime setup/smoke execution, skill publishing, and regression tests are isolated behind focused modules. The 302-line root is a thin command/status facade, and the 466-line validation module is partitioned into explicit load, metadata, skill, support-file, and projection phases. |
| Desktop LM Studio auth | `app/desktop/src-tauri/src/auth/lm_studio.rs` | Parser and environment boundaries remain isolated, but later load-context work has grown the root to 1,124 lines. This non-#838 hotspot remains owned by #235; do not describe it as below threshold. |
| Canonical sessions | `app/desktop/src-tauri/src/canonical_sessions.rs` | Canonical regressions, typed read models, persistence resources, identity migration, group policy, message lookup, and title policy are isolated. The 465-line composition root and 37-line `commands.rs` facade retain stable Tauri exports. |
| Cloud collaboration state | `app/desktop/src/features/cloud/useCloudCollaborationState.ts` | Availability, execution, self-agent sync, transport, reconciliation, account, catalog, session action, group control, message sync, and read-model lifecycles are owned by focused modules. The current composition hook is 769 lines. |
| Frontend app model | `app/desktop/src/app/useKordiAppModel.ts` | Domain callbacks and state owners are composed through focused controllers. The exported composition hook is 31 lines. |
| Kordi app type barrel | `app/desktop/src/kordi-app/types.ts` | Message-focused types remain extracted, but later product contracts have grown the barrel to 1,191 lines. Further frontend type partitioning remains owned by #235. |
| Transcript UI | `app/desktop/src/kordi-app/components/transcript.tsx` | Attachment and live-turn rendering have focused modules, but the current 1,627-line root still owns message chrome and contact rows. Further frontend work remains owned by #235. |
| Workspace sidebar | `app/desktop/src/pages/WorkspaceSidebar.tsx` | Navigation, chat, profile, updater, and support-panel surfaces are split into focused modules. The remaining component is 431 lines and consumes cohesive contracts. |
...
| Chats page | `app/desktop/src/pages/ChatsPage.tsx` | Main/companion workspaces, pane primitives, controllers, routing, and composer behavior are split by interaction surface. The remaining component is 357 lines and consumes cohesive contracts. |
| Desktop chat state | `app/desktop/src/features/chat/useDesktopChatState.ts` | Queue persistence, live-turn snapshots, and refresh/cache reducers are extracted. The current 538-line hook retains polling/completion orchestration and notification side effects. |
| Desktop API client | `app/desktop/src/lib/desktop.ts` | The current 1,065-line compatibility facade remains a #235 hotspot. Group exports by resource only when consumers can move in the same PR. |
| Bridges CLI commands | `bridges/cli/src/commands.rs` | Setup/runtime selection, project lifecycle, identity lifecycle, and doctor diagnostics are extracted. The 833-line root retains the remaining command-family dispatch and stable exports. |
...
| Bridges relay | `bridges/cli/src/serve/relay.rs` | The 19 relay regressions are partitioned into acknowledgement, authorization, persistence, and realtime scenario modules. The 814-line production module owns HTTP/WebSocket relay routes and durable mailbox persistence without inline-test inflation. |
...
| Cloud Agent runs | `bridges/cloud-server/src/cloud_agent_runtime/runs.rs` | The 304-line root composes stable exports; authorization, envelopes, prompts, claims, leases, durable delivery, terminal transitions, and typed domain failures each have one focused child module. HTTP mapping remains at the route boundary. |
| Local app server | `app/server/src/lib.rs` | The 112-line root retains only state construction, route composition, and serving. HTTP mapping, typed session projection, protocol mapping, Bridge health, configuration, turn execution, and partitioned regressions live in focused modules. |
| Authentication storage | `agent/crates/cli/src/login/store.rs` | The 173-line facade composes serialized models, generic profile selection/updates, legacy migration, atomic persistence, GitHub Copilot state, and external regressions behind the existing auth-store API. |
| Provider errors | `agent/crates/provider/src/error.rs` | Typed failures, safe upstream diagnostics, and retry policy remain one cohesive 958-line production module. Its 12 regressions are partitioned into parsing/sanitization, response-handling, and retry-policy scenarios. |
| CLI session bootstrap | `agent/crates/cli/src/session_bootstrap.rs` | The cohesive production bootstrap root is 805 lines. Its 15 option, selection, prompt-construction, and session-resolution regressions live in a dedicated sibling test module. |
| OAuth credential resolution | `agent/crates/cli/src/login/resolver/oauth_refresh.rs` | The 657-line production module owns credential resolution and synchronous refresh coordination. Its eight precedence, expiry, and persisted-choice regressions live in a dedicated sibling test module. |
| Request metrics tracker | `agent/crates/kordi-monitor/src/request_metrics/tracker.rs` | The 463-line production root owns request-metrics state transitions and final observation construction. Its nine regressions are partitioned into cache usage, final metrics, parity, and tracker-state scenarios. |
...
## Rust completion snapshot (#838)

The #838 baseline is commit `10f2c8f65e0641ebfe9fe52cbfe155de44268329`.
The current measurement counts newline-delimited LOC in git-tracked `*.rs` files.
Paths named `tests`, `tests.rs`, `test.rs`, `*_test.rs`, or `*_tests.rs` are test
paths; inline `#[cfg(test)]` modules intentionally remain part of the production-path
number, matching the issue baseline methodology.
...
| #838 P1 boundary | Final root LOC | Root responsibility |
| --- | ---: | --- |
| `canonical_sessions.rs` / `canonical_sessions/commands.rs` | 465 / 37 | Persistence-service and Tauri-command composition facades |
| `chat/agent_builder.rs` | 302 | Factory command and status projection facade |
| `cloud_agent_runtime/runs.rs` | 304 | Cloud run resource composition and stable exports |
| `desktop_runtime.rs` | 915 | Desktop session runtime composition and configuration |
| `chat.rs` | 685 | Stable Tauri chat command facade |
| `app/server/src/lib.rs` | 112 | Axum state, router, and server composition |
| `login/store.rs` | 173 | Stable authentication-store facade |

No non-test function modified by #838 exceeds 200 lines. The 260-line
`prepare_session_runtime_for_cwd` function appears in a P2 file whose production body
is byte-for-byte unchanged from the baseline; #838 only moved that file's inline
tests. It remains a linear startup composer owned by `session_bootstrap.rs`, rather
than a retained exception to this refactor.
...
- `app/server/src/lib.rs`: the nine bootstrap, session, fork, and turn endpoint regression scenarios moved into the `app/server/src/tests/` module tree; scenario names and assertions remain intact while production behavior stays untouched.
- `app/server/src/tests.rs`: app-server regressions are partitioned into bootstrap/session/fork and turn-lifecycle scenario modules, with shared fakes and fixtures retained in the test root; all test files remain below the 500-line growth guardrail and scenario assertions are unchanged.
...
- `app/desktop/src-tauri/src/canonical_sessions.rs`: delegated-exchange deduplication, upsert, status defaults, Bridge correlation fields, and row projection moved to `app/desktop/src-tauri/src/canonical_sessions/persistence/delegated_exchanges.rs`. Existing IDs, SQL conflict behavior, cleanup rules, catalog lookup paths, and error strings remain unchanged.
- `app/desktop/src-tauri/src/canonical_sessions/commands.rs`: message append/upsert handlers, delivery-state validation and atomic delta updates, and delegated-exchange creation moved to `app/desktop/src-tauri/src/canonical_sessions/commands/delivery.rs`. Tauri-facing function paths, request/response DTOs, transaction behavior, validation strings, and state-refresh semantics remain unchanged.
- `app/desktop/src-tauri/src/canonical_sessions/commands.rs`: participant row projection, group rename/metadata authorization, single and batched membership changes, join notices, removal, and admin-role commands moved to `app/desktop/src-tauri/src/canonical_sessions/commands/groups.rs`. Creator/admin/member rules, atomic multi-session invitation, metadata merge precedence, join-event IDs/messages, and facade paths remain unchanged.
- `app/desktop/src-tauri/src/canonical_sessions/commands.rs`: identity adoption/upsert, session open/archive/delete, presence, and read-cursor command orchestration moved to `app/desktop/src-tauri/src/canonical_sessions/commands/lifecycle.rs`. The root is now a thin `catalog`/`delivery`/`groups`/`lifecycle` facade; Tauri command paths, SQL order, return DTOs, and error strings remain unchanged.
...
- `bridges/cli/src/serve/relay.rs`: the 19 inline relay tests moved to scenario-owned modules under `bridges/cli/src/serve/relay/tests/`, with shared fixtures retained in `tests.rs`; every scenario file stays below 500 lines and the production root is 814 lines.
- `agent/crates/provider/src/error.rs`: the 12 inline provider-error tests moved to parsing/sanitization, response-handling, and retry-policy modules under `agent/crates/provider/src/error/tests/`; the cohesive production root is 958 lines and every scenario file stays below 500 lines.
- `agent/crates/cli/src/session_bootstrap.rs`: the 15 inline bootstrap regressions moved to `agent/crates/cli/src/session_bootstrap/tests.rs`, leaving an 805-line production root and a 376-line focused test module.
- `agent/crates/cli/src/login/resolver/oauth_refresh.rs`: the eight inline OAuth-resolution regressions moved to `agent/crates/cli/src/login/resolver/oauth_refresh/tests.rs`, leaving a 657-line production root and a focused test module below 500 lines.
- `agent/crates/kordi-monitor/src/request_metrics/tracker.rs`: the nine inline tracker regressions moved under `agent/crates/kordi-monitor/src/request_metrics/tracker/tests/` by cache usage, final metrics, parity, and tracker state; the production root is 463 lines and every scenario file stays below 200 lines.
...
- `app/desktop/src-tauri/src/chat.rs`: background message-turn setup, provider readiness, streaming, completion/cancellation, canonical synchronization, and failure projection moved behind the typed request in `app/desktop/src-tauri/src/chat/message_execution.rs`; the public Tauri signature and turn snapshot behavior remain unchanged.
- `app/desktop/src-tauri/src/chat/agent_builder/validation.rs`: workspace validation is partitioned into file loading, agent metadata, declared skills, support-file limits, and draft projection phases; validation order, error text, file status, fingerprints, and draft shapes remain unchanged.
...
- `app/desktop/src-tauri/src/auth/lm_studio.rs`: environment DTO assembly, LM Studio app/home/bin discovery, CLI path resolution, shell PATH repair helpers, and plist parsing moved to `app/desktop/src-tauri/src/auth/lm_studio/environment.rs`; later load-context growth is recorded separately in the current disposition table.
...
- `app/desktop/src/kordi-app/types.ts`: message, transcript artifact, queued-message, and live-turn snapshot types moved to `app/desktop/src/kordi-app/types/message.ts`, with root re-exports preserving existing imports and a boundary test covering the split; later type growth is recorded separately in the current disposition table.
