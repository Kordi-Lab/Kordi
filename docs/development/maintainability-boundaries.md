...
| Canonical sessions | `app/desktop/src-tauri/src/canonical_sessions.rs` | Canonical session tests are partitioned by scenario; identity/session persistence and migration, identity/session request helpers, group participant/admin mutations, message lookup/dedup, and title reconciliation policy have been extracted. The 855-line composition root is below the 1,000-line scan threshold. |
