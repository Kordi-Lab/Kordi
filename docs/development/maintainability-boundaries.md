...
| Desktop Agent Builder | `app/desktop/src-tauri/src/chat/agent_builder.rs` | Models, durable storage, workspace validation/fingerprinting, and atomic workspace lifecycle are isolated behind focused modules. The 804-line composition/command root is below the 1,000-line scan threshold; runtime and publishing orchestration remain named follow-up boundaries. |
