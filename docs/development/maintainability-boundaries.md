...
| Local app server | `app/server/src/lib.rs` | Regression tests and process-based turn execution are isolated behind focused modules. Production extraction is proceeding by stable HTTP, session projection, Bridge health, and configuration boundaries so the root can retain only server composition. |
