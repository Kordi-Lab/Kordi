...
| Authentication storage | `agent/crates/cli/src/login/store.rs` | The persisted-auth regression suite is isolated in `login/store/tests.rs`. Generic profile selection, migration, persistence, and GitHub Copilot configuration are being separated behind stable auth-store contracts. |
