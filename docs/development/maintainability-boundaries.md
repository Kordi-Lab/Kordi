...
| Local app server | `app/server/src/lib.rs` | Regression tests, process-based turn execution, and environment/filesystem configuration resolution are isolated behind focused modules. Production extraction is proceeding by stable HTTP, session projection, Bridge health, and protocol boundaries so the root can retain only server composition. |
