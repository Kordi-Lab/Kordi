...
Chat bootstrap includes account visibility in the same database snapshot as its cursor. Clients restore visibility with the cached cursor before rendering the conversation catalog; history backfill does not hold the catalog open with unknown visibility. Authentication updates are independent of that history backfill. A fresh device without local provider authentication remains unauthenticated and must not publish empty provider settings or implicitly replace Cloud credentials. Successful explicit local authentication refreshes Cloud's provider snapshot and its source device. Explicit local provider logout/removal retains account-wide Cloud revocation semantics, including snapshots published by another device. Authentication intents are scoped to their originating account and device, and account changes invalidate pending local intent.
