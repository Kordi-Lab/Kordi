...
The implementation review corrected device reads overwriting shared generation preferences, reuse of previous claims after their source message was edited, the generation timeout waiting indefinitely inside a lease-renewal request, and iOS display of a start-only all-day event. Calendar additions now enforce the same 1,000-event bound as reads, while existing events remain editable at capacity. Generated owner validation and task owner choices exclude identities supported only by agent-authored messages.

Regression checks include stalled renewal under virtual time, source-version invalidation, shared-preference initialization, calendar capacity and edits at capacity, and single-day all-day rendering. The PR screenshots render the actual desktop components using the earlier synthetic model-evaluation snapshot; they contain no imported personal calendar contents.
