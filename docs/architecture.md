...
Environment choice is mandatory before a preview or debug session. Product-server-affecting operator work must be developed and tested on the corresponding product-server machine through `https://kordi.ai`; desktop-only remote operator previews use the same canonical origin through the allowlisted launcher. Isolated contributor environments remain separate and cannot substitute for product-server validation. See the [required environment preflight](hosted-cloud-developer-guide.md#required-preflight-before-preview-or-debug).
