...
| iOS Cloud API | `app/ios/Kordi/Core/API/NetworkProxyPolicy.swift` classifies proxy state and failures; `CloudAPIClient` uses a proxy-disabled direct session for loopback, retries pre-connection failures once directly, and surfaces an actionable proxy error otherwise. |
