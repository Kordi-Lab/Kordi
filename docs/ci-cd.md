...
| macOS reference | `macos-15` | Visual baselines and Apple/Tauri compilation. |
| Xcode | the `macos-15` image default | Unsigned iOS simulator compilation only. No signing or distribution credentials exist in these jobs. |
...
| `ios` | `pnpm check:ios` | `macos-15` | Unsigned iOS simulator compilation (`app/ios`, `Kordi Beta` scheme, `CODE_SIGNING_ALLOWED=NO`; no signing credentials). |
