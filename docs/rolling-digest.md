...
Opening the authenticated `/v1/cloud/digest` endpoint enables the account's monitor and initializes its language and timezone. Later reads preserve these shared preferences; an explicit refresh adopts the requesting device's language and timezone, avoiding repeated generation when two devices have different settings. A five-second worker sweep compares authorized source content, session metadata, task state and calendar state. Only changed inputs enqueue a run, and an atomic account-row reservation prevents concurrent generations. Changes during a run are picked up after it finishes. The current snapshot and its evidence remain separate from the in-flight input.
...
- Desktop Brief entries and suggestions can be dismissed independently. Dismissal is saved for the account and can be restored from the corresponding view.
