...
type ParticipantProfile = (DateTime<Utc>, String);
...
fn enrich_participant(
    ...
    let Some((joined_at, agent_name)) = profiles.get(&account_id) else {
        return;
    };
    participant.insert(
        "joinedAt".to_string(),
...
pub(super) async fn normalize_group_envelope(
    ...
    let rows: Vec<(String, DateTime<Utc>, String)> = query_as(
        "SELECT member.account_id, member.joined_at, agent.display_name \
    ...
    let profiles = rows
        .into_iter()
        .map(|(account_id, joined_at, agent_name)| (account_id, (joined_at, agent_name)))
        .collect::<HashMap<_, _>>();
...
mod tests {
    ...
    fn repairs_legacy_default_agent_names_without_relabeling_custom_agents() {
        ...
        normalize_stored_group_agent_identity(&mut legacy, "acct_owner", "Kordirename11");
        ...
        normalize_stored_group_agent_identity(&mut custom, "acct_owner", "Kordirename11");
