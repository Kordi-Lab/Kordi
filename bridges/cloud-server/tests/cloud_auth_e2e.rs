...
async fn cloud_sync_returns_message_events_after_cursor() {
    ...
    let signup = router.clone().oneshot(post("/v1/cloud/auth/signup", signup_body(&email, "correct horse"))).await.unwrap();
    ...
    let peer_signup = router.clone().oneshot(post("/v1/cloud/auth/signup", signup_body(&peer_email, "correct horse"))).await.unwrap();
    ...
    let send_resp = router.clone().oneshot(post_json_with_token(
        "/v1/cloud/messages",
        &token,
    ...
    let sync_resp = router.clone().oneshot(get_with_token("/v1/cloud/sync?cursor=0", &token)).await.unwrap();
    ...
    assert_eq!(sync["events"][0]["payload"]["message"]["body"], "diff hello");
    assert!(sync["cursor"].as_str().unwrap().parse::<i64>().unwrap() > 0);

    let peer_sync_resp = router.oneshot(get_with_token("/v1/cloud/sync?cursor=0", &peer_token)).await.unwrap();
...
async fn cloud_sync_paginates_and_advances_cursor() {
    ...
    let page1_resp = router.clone().oneshot(get_with_token("/v1/cloud/sync?cursor=0&limit=1", &token)).await.unwrap();
    ...
    let page2_resp = router.oneshot(get_with_token(&format!("/v1/cloud/sync?cursor={cursor1}&limit=1"), &token)).await.unwrap();
    ...
    assert!(page2["cursor"].as_str().unwrap().parse::<i64>().unwrap() > cursor1.parse::<i64>().unwrap());
}
...
async fn cloud_sync_returns_read_receipt_events() {
    ...
    let read_resp = router.clone().oneshot(post_json_with_token(
    ...
    let sync_resp = router.oneshot(get_with_token("/v1/cloud/sync?cursor=0", &token)).await.unwrap();
    assert_eq!(sync_resp.status(), StatusCode::OK);
    let sync = read_json(sync_resp).await;
    let read_event = sync["events"].as_array().unwrap().iter().find(|event| event["eventType"] == "message.read").unwrap();
    assert_eq!(read_event["peerAccountId"], peer_account_id);
    assert_eq!(read_event["payload"]["readerAccountId"], peer_account_id);
    assert_eq!(read_event["payload"]["messageIds"].as_array().unwrap(), &vec![json!(message_id)]);
}
