...
async fn owner_can_stop_from_another_device_but_members_and_stale_executions_cannot() {
    ...
    // A publisher that learns the new version still cannot revive Stop with
    // a late successful result or a stale running snapshot.
    sqlx_core::query::query("UPDATE cloud_agent_subsessions SET publisher_device_id=(SELECT device_id FROM cloud_devices WHERE account_id=$2 LIMIT 1) WHERE subsession_id=$1")
        .bind(id).bind(&owner.account_id).execute(&pool).await.unwrap();
    for status in ["done", "running"] {
        let late = router.clone().oneshot(Request::builder().method("PUT").uri(&uri_read)
            .header("authorization", format!("Bearer {}", owner.token))
            .header("content-type", "application/json")
            .body(Body::from(json!({
                "parentSessionId": parent_id, "parentRequestId": "initial", "title": "Stop fixture",
                "status": status, "messages": [], "expectedVersion": stopped["version"]
            }).to_string())).unwrap()).await.unwrap();
        assert_eq!(late.status(), StatusCode::CONFLICT);
        assert_eq!(
            read_json(late).await["error"]["code"],
            "subsession_is_terminal"
        );
    }
