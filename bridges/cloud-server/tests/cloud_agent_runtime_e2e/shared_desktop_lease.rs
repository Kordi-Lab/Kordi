...
async fn shared_desktop_lease_resolves_ids_and_publishes_once() {
    ...
    for group in [true, false] {
        ...
        let same = router
            ...
            .unwrap();
        assert_eq!(
            read_json(same).await["turnIdentity"],
            claimed["turnIdentity"],
            "retries must reuse the admitted snapshot after a rename"
        );
        sqlx_core::query::query(
            "UPDATE cloud_accounts SET display_name='Owner' WHERE account_id=$1",
        )
        .bind(&owner.account_id)
        .execute(&pool)
        .await
        ...
        let replay = router
            ...
            .oneshot(post_json_with_token(
                "/v1/cloud/agent-runs/desktop/claim",
                &owner.token,
                input(canonical, uuid::Uuid::new_v4()),
            ))
            .await
            .unwrap();
        ...
        let admitted = router
            .clone()
            .oneshot(post_json_with_token(
                &format!("/v1/cloud/agent-runs/desktop/{run}/admit"),
                &owner.token,
