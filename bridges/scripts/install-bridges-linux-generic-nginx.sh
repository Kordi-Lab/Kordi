...
install_packages() {
  ...
  install_bridges_packages \
    "${pm}" \
    "Unsupported package manager. Install curl, git, tar, a C toolchain, pkg-config, OpenSSL development headers, nginx, and optionally certbot manually." \
    "${packages[@]}"
}
...
enable_base_services() {
  enable_bridges_and_proxy_service Nginx nginx
}
