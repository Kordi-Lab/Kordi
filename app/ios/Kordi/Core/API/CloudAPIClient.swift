...
actor CloudAPIClient {
    ...
    private func transportValue<Value>(
        ...
        let primaryRoute = primarySession === directSession ? .direct : proxyStateProvider()
        do {
            ...
            guard primarySession !== directSession,
                ...
                throw Self.transportError(error, route: primaryRoute, fallback: fallback)
            }
            do {
                ...
                throw Self.transportError(error, route: .direct, fallback: fallback)
            }
        }
    }
    ...
    private static func transportError(
        _ error: Error,
        route: NetworkProxyPolicy.SystemState,
        fallback: String
    ) -> CloudAPIError {
        let proxyState = NetworkProxyPolicy.failureState(for: error, route: route)
