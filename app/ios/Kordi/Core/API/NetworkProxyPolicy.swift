...
enum NetworkProxyPolicy {
    ...
    /// System configuration alone does not show that a request used a proxy.
    /// Require a proxy-specific CFNetwork error on a system-routed attempt.
    static func failureState(for error: Error, route: SystemState) -> SystemState {
        guard route != .direct else { return .direct }
        var cause: NSError? = error as NSError
        for _ in 0..<8 {
            guard let current = cause else { break }
            if current.domain == kCFErrorDomainCFNetwork as String,
               [CFNetworkErrors.cfErrorHTTPProxyConnectionFailure.rawValue,
                ...
                CFNetworkErrors.cfErrorPACFileAuth.rawValue].contains(where: { Int($0) == current.code }) {
                return route
            }
