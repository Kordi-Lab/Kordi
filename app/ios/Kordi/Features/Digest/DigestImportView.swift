    ...
    var body: some View {
        Form {
            ...
            if let result {
                ...
                Button("Import selected events") { Task { busy = true; defer { busy = false }; do { report = try await save(result.events.filter { selected.contains($0.id) }) } catch { self.error = error.localizedDescription } } }.disabled(busy || selected.isEmpty)
...
private struct DigestImportCompletion: View {
    ...
    var body: some View {
        Section("Import complete") { Text("\(report.imported) events imported."); if report.duplicates > 0 { Text("\(report.duplicates) already in your calendar.").foregroundStyle(.secondary) } }
        if !report.skipped.isEmpty { Section("Skipped events") { ForEach(Array(report.skipped.enumerated()), id: \.offset) { _, reason in Text(reason).font(.subheadline) } } }
        Button("Done") { dismiss() }
    }
}
