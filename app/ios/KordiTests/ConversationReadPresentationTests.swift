...
final class ConversationReadPresentationTests: XCTestCase {
    ...
    func testMessageBubblesUseThemeAwareContrastForRepliesLinksAndMentions() throws {
        ...
        XCTAssertFalse(bubbleSource.contains("replyPreviewBackgroundColor"))
        XCTAssertTrue(bubbleSource.contains("quoteLine(source)"))
    ...
    func testThreadRepliesShareOneAccessoryRowAndCannotNest() throws {
        ...
        XCTAssertTrue(bubbleSource.contains("!reactions.isEmpty || threadReplyCount > 0 || quoteReplyCount > 0"))
        ...
        XCTAssertTrue(conversationSource.contains(".background(NavigationMinimalBackButtonBridge())\n                .navigationDestination(item: $activeRootMessageID)"))
        XCTAssertFalse(conversationSource.contains(".toolbarRole(.editor)"))
        XCTAssertTrue(conversationSource.contains("|| (scopedThreadRootMessageID == nil && model.hasEarlierMessages(for: conversation))"))
        XCTAssertTrue(conversationSource.contains("scrollPosition.setOffsetY(targetOffsetY, animated: !reduceMotion)"))
        XCTAssertTrue(conversationSource.contains("let started = scrollPosition.jumpToMessage("))
        XCTAssertTrue(bubbleSource.contains(".fill(chatTheme.accent.opacity(isHighlighted && !isSelected ? 0.16 : 0))"))
        XCTAssertFalse(bubbleSource.contains("isHighlighted && !isSelected && !isActionPresented ? 1.018 : 1"))
        XCTAssertTrue(conversationSource.contains("controller.navigationItem.backButtonDisplayMode = .minimal"))
        ...
        XCTAssertTrue(actionSource.contains("actionButton(\"Quote\", systemImage: \"text.quote\")"))
        XCTAssertTrue(actionSource.contains("actionButton(\"Open discussion\", systemImage: \"bubble.left.and.bubble.right\")"))
        XCTAssertFalse(actionSource.contains("Reply in conversation"))
        XCTAssertFalse(actionSource.contains("Reply in thread"))
    ...
    func testMessageActionsAllowManualTextSelectionOnlyAfterOpening() throws {
        ...
        XCTAssertFalse(bubbleSource.contains("isHighlighted || isSelected"))
