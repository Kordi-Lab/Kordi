...
pub(crate) struct Http {
    default_client: std::sync::OnceLock<reqwest::Client>,
    #[cfg(feature = "cookies")]
    cookies_jar: std::sync::Arc<crate::reqwest_cookie_store::CookieStoreMutex>,
}

impl Http {
    /// Client options are request-local; only the default configuration is pooled.
    fn client(
        &self,
        ...
        // Racing first requests may build twice, but all use the winning pool.
        let client = builder.build()?;
        let _ = self.default_client.set(client);
        Ok(self
            .default_client
            .get()
...
pub fn init<R: Runtime>() -> TauriPlugin<R> {
    Builder::<R>::new("http")
        .setup(|app, _| {
            ...
            let state = Http {
                default_client: std::sync::OnceLock::new(),
                #[cfg(feature = "cookies")]
