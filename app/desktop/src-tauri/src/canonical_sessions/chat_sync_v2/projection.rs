...
pub(super) fn required_text<'a>(value: &'a Value, key: &str) -> Result<&'a str, String> {
    value
        ...
        .ok_or_else(|| format!("Chat sync v2 snapshot is missing {key}"))
}
...
pub(super) fn upsert_conversation(
    ...
    if version < 1 {
        return Err("Chat sync v2 conversation version is invalid".to_string());
    }
    ...
    tx.execute(
        "INSERT INTO chat_sync_v2_conversations
         ...
         WHERE excluded.version >= chat_sync_v2_conversations.version",
        params![
            account_id,
            conversation_id,
            client_session_id,
...
pub(super) fn upsert_message(
    ...
    if sequence < 1 || version < 1 {
        return Err("Chat sync v2 message sequence or version is invalid".to_string());
    }
    ...
    tx.execute(
        "INSERT INTO chat_sync_v2_messages
         ...
         ON CONFLICT(account_id, message_id) DO UPDATE SET
             ...
             snapshot_json = excluded.snapshot_json,
             updated_at_ms = excluded.updated_at_ms
         WHERE excluded.version >= chat_sync_v2_messages.version",
        params![
            account_id,
            message_id,
            conversation_id,
...
pub(super) fn merge_conversation_projection(
    ...
    let Some(stored) = stored else {
        return Err("Chat sync v2 event references an unknown conversation".to_string());
    };
    let mut snapshot: Value = serde_json::from_str(&stored).map_err(|error| error.to_string())?;
    let object = snapshot
        .as_object_mut()
        .ok_or_else(|| "Stored chat sync v2 conversation is invalid".to_string())?;
    update(object);
    let encoded = serde_json::to_string(&snapshot).map_err(|error| error.to_string())?;
    tx.execute(
        "UPDATE chat_sync_v2_conversations
...
pub(super) fn apply_event(
    ...
    let payload = event
        ...
        .ok_or_else(|| "Chat sync v2 event payload is invalid".to_string())?;
    if event_type == "membership.removed" {
        ...
        if removed_account_id == account_id {
            tx.execute(
                "DELETE FROM chat_sync_v2_messages
                 WHERE account_id = ?1 AND conversation_id = ?2",
                params![account_id, conversation_id],
            )
            .map_err(|error| error.to_string())?;
            tx.execute(
                "DELETE FROM chat_sync_v2_conversations
                 WHERE account_id = ?1 AND conversation_id = ?2",
                params![account_id, conversation_id],
            )
            .map_err(|error| error.to_string())?;
    ...
    if event_type == "conversation.preferences.updated" {
        ...
        let preferences = payload
            .get("preferences")
            .cloned()
            .ok_or_else(|| "Chat sync v2 preference event is invalid".to_string())?;
    ...
    if matches!(
        ...
        let cursor = payload
            .get("cursor")
            .and_then(Value::as_object)
            .ok_or_else(|| "Chat sync v2 cursor event is invalid".to_string())?;
