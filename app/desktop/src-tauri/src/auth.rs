...
pub fn desktop_cloud_provider_auth_snapshot_payload(
    ...
    let requested_auth_choice = auth_choice
        ...
        .filter(|value| !value.is_empty())
        .map(ToString::to_string);
    let resolver_auth_choice =
        cloud_provider_auth_explicit_choice(requested_auth_choice.as_deref());
    ...
    let model = cloud_provider_auth_snapshot_model(provider, model.as_deref());
    ...
    let credential_revision =
        cloud_provider_auth_credential_revision(&provider, &auth_choice, &payload);
...
fn cloud_provider_auth_explicit_choice(auth_choice: Option<&str>) -> Option<&str> {
    auth_choice
        .map(str::trim)
        .filter(|choice| choice.starts_with("profile:") || choice.starts_with("env:"))
}

fn cloud_provider_auth_credential_revision(
    provider: &str,
    auth_choice: &str,
...
fn cloud_provider_auth_snapshot_model(provider: &str, model: Option<&str>) -> String {
    model
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(ToString::to_string)
...
mod tests {
    use super::{
        cloud_provider_auth_credential_revision, cloud_provider_auth_explicit_choice,
        cloud_provider_auth_snapshot_model,
    };

    #[test]
    fn cloud_auth_snapshot_resolves_legacy_default_through_active_auth() {
        assert_eq!(cloud_provider_auth_explicit_choice(Some("default")), None);
        assert_eq!(
            cloud_provider_auth_explicit_choice(Some("local-active-oauth")),
            None
        );
        assert_eq!(
            cloud_provider_auth_explicit_choice(Some("profile:account-one")),
            Some("profile:account-one")
        );
        assert_eq!(
            cloud_provider_auth_explicit_choice(Some("env:api-key")),
            Some("env:api-key")
        );
    }

    #[test]
    fn cloud_auth_snapshot_uses_provider_specific_default() {
        assert_eq!(
            cloud_provider_auth_snapshot_model("openai", None),
            "gpt-5.6-sol"
        );
        assert_eq!(
            cloud_provider_auth_snapshot_model("anthropic", None),
            "claude-opus-4-6"
        );
        assert_eq!(
            cloud_provider_auth_snapshot_model("github-copilot", None),
            "gpt-5.4"
        );
        assert_eq!(
            cloud_provider_auth_snapshot_model("openai", Some("gpt-5.4")),
            "gpt-5.4"
        );
    }

    #[test]
    fn cloud_auth_snapshot_revision_changes_with_route_or_credential() {
        let first = cloud_provider_auth_credential_revision(
            "anthropic",
            "profile:one",
            &serde_json::json!({
        ...
        let same = cloud_provider_auth_credential_revision(
            "anthropic",
            "profile:one",
            &serde_json::json!({
                "accessToken": "access-one",
        ...
        let refreshed = cloud_provider_auth_credential_revision(
            "anthropic",
            "profile:one",
            &serde_json::json!({
                "accessToken": "access-one",
        ...
        let inactive = cloud_provider_auth_credential_revision(
            "anthropic",
            "profile:one",
            &serde_json::json!({
                "accessToken": "access-one",
