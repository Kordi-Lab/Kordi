...
impl ChildAgentRunner for ManagedChildAgentRunner {
    async fn spawn(&self, request: SpawnRequest) -> Result<SpawnedTask> {
        ...
        let session_id =
            create_background_session(&request.cwd, &self.parent_session_id, &request.task_title)?;
        let runtime =
        ...
        let turn = match self.start_turn(session_id.clone(), request.message).await {
            Ok(turn) => turn,
            Err(error) => {
    ...
    async fn send(&self, target: &str, message: String) -> Result<()> {
        ...
        let turn = self.start_turn(task.session_id.clone(), message).await?;
        self.manager
            .background_turn_ids
            .lock()
            .await
