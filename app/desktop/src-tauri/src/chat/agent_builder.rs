...
fn checked_draft_id(value: &str) -> Result<&str, String> {
    let trimmed = value.trim();
    uuid::Uuid::parse_str(trimmed).map_err(|_| "Kordi Factory draft id is invalid".to_string())?;
    Ok(trimmed)
}

fn container_for_draft(draft_id: &str) -> Result<PathBuf, String> {
    Ok(drafts_root().join(checked_draft_id(draft_id)?))
}

fn workspace_for_draft(draft_id: &str) -> Result<PathBuf, String> {
    Ok(container_for_draft(draft_id)?.join(WORKSPACE_DIR))
}

fn draft_container(workspace: &Path) -> Result<&Path, String> {
    workspace
        .parent()
        .ok_or_else(|| "Kordi Factory workspace is invalid".to_string())
}

fn metadata_path(container: &Path) -> PathBuf {
    container.join(METADATA_FILE)
}

fn test_report_path(container: &Path) -> PathBuf {
    container.join(TEST_REPORT_FILE)
}

fn resources_root(container: &Path) -> PathBuf {
    container.join(RESOURCES_DIR).join("skills")
}

fn write_metadata(workspace: &Path, metadata: &DesktopAgentBuilderMetadata) -> Result<(), String> {
...
fn find_active_draft(target_key: &str) -> Option<DesktopAgentBuilderMetadata> {
...
fn new_metadata(target_key: &str) -> DesktopAgentBuilderMetadata {
...
fn read_test_report(container: &Path) -> Option<DesktopAgentBuilderTestReport> {
...
fn load_metadata(draft_id: &str) -> Result<(PathBuf, DesktopAgentBuilderMetadata), String> {
    let container = container_for_draft(draft_id)?;
    let workspace = workspace_for_draft(draft_id)?;
