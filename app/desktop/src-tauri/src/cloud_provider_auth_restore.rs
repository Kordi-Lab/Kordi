...
struct RestoreResponse {
    ...
    changed: bool,
    envelope: Option<RestoreEnvelope>,
}
...
async fn request_restore_envelope(
    token: &str,
...
fn provider_auth_device_secret(account_id: &str) -> Result<StaticSecret, String> {
...
fn required_string(payload: &serde_json::Map<String, Value>, key: &str) -> Result<String, String> {
    payload
        .get(key)
        .and_then(Value::as_str)
        .map(str::trim)
...
fn optional_string(payload: &serde_json::Map<String, Value>, key: &str) -> String {
    payload
        .get(key)
        .and_then(Value::as_str)
        .map(str::trim)
...
fn optional_value(payload: &serde_json::Map<String, Value>, key: &str) -> Option<String> {
    let value = optional_string(payload, key);
    (!value.is_empty()).then_some(value)
}

fn required_i64(payload: &serde_json::Map<String, Value>, key: &str) -> Result<i64, String> {
    payload
        .get(key)
