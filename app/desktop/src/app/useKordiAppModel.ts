...
export function useKordiAppModel({
  ...
  const onForwardMessage = useCallback((message: Message) => {
    const source = forwardMessageSourceFromMessage(message, activeConv.canonicalSessionId ?? activeConv.id ?? chatDraftSessionId);
    if (!source) return;
  ...
  const sourceForSelectableMessage = useCallback((message: Message) => (
    forwardMessageSourceFromMessage(message, activeConv.canonicalSessionId ?? activeConv.id ?? chatDraftSessionId)
  ...
  const handleCreateChatSessionInParticipantSpace = useCallback(async (space: ParticipantSpaceViewModel) => {
    ...
    if (space.kind === 'group') {
      const existingDraft = participantSpaceDraftByKeyRef.current.get(createKey);
      if (existingDraft) {
        setActiveNav('chats');
        setActiveConvId(existingDraft.sessionId);
        return;
      }
      ...
      const groupSpaceId = metadataGroupSpaceId(sourceMetadata) || normalizeStoredGroupSpaceId(space.id) || sourceSessionId || sessionId;
      const metadata = {
        ...groupSourceMetadata,
        schemaVersion: 1,
        kind: 'chat-group',
      ...
      const conversation: Conversation = {
        ...
        avatarSeed: primaryParticipant?.avatarKey ?? primaryParticipant?.id ?? null,
      ...
      participantSpaceDraftByKeyRef.current.set(createKey, draft);
      participantSpaceDraftBySessionIdRef.current.set(sessionId, draft);
      setParticipantSpaceDrafts((current) => [...current.filter((candidate) => candidate.createKey !== createKey), draft]);
      selectNewChatSession(sessionId);
      return;
    }
