...
export { cloudGroupTitleUpdateNoticeRequest, cloudTitleUpdateNoticeEquivalent, cloudSessionTitleUpdateNoticeRequest } from './cloudGroupNotices';
...
export function parseCloudGroupControl(body: string): CloudGroupControlEnvelope | null {
  ...
  try {
    ...
    return {
      ...
      ...(kind === 'group-invite' && parsed.channelCreated === true ? { channelCreated: true } : {}),
      ...(memberJoins.length > 0 ? { memberJoins } : {}),
      ...(memberLeaves.length > 0 ? { memberLeaves } : {}),
      message,
    };
