...
export function createCanonicalSessionReadModel(
  ...
  return {
    ...
    applyConversation(conversation, buildSubtitle) {
      ...
      const normalizedMessages = isSupportContact
        ? normalizeSupportContactMessages(hydratedMessages)
        : hydratedMessages;
      const messages = dedupeRepeatedFailedAgentTurns(normalizedMessages);
