...
export async function transcribeDesktopVoiceMessage(path: string, locale?: string) {
  ...
  for (const candidate of locales) {
    try {
      return await invokeDesktop<string>('desktop_voice_transcribe', {
        path,
        locale: candidate,
      });
    } catch (error) {
...
export async function trimDesktopVoiceMessage(path: string, startMs: number, endMs: number) {
  return invokeDesktop<string>('desktop_voice_trim', { path, startMs, endMs });
}

export type DesktopVoiceRecordingSample = {
  durationMs: number;
  level: number;
};

export type DesktopVoiceRecordingStop = {
  path: string;
  durationMs: number;
  sizeBytes: number;
};
...
export function sampleDesktopVoicePlayback(path: string) {
  return invokeDesktop<DesktopVoicePlaybackSample>('desktop_voice_playback_sample', { path });
}
