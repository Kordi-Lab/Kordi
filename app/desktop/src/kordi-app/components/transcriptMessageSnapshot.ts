...
export function messageSnapshotKey(msg: Message) {
  return [
    ...
    JSON.stringify(msg.messageAction ?? null),
    ...
    msg.threadSummary?.unread ? 'thread-unread' : '',
    msg.conversationSequence ?? '',
    msg.threadSummary?.agentState ?? '',
    ...
    msg.planCard ? [msg.planCard.eventId, msg.planCard.revision, ...(msg.planCard.options ?? []).map((option) => option.votes.join('+'))].join(':') : '',
    msg.editedAt ?? '',
