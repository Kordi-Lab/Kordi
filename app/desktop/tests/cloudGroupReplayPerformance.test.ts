...
test('group member removal publishes the current membership and a durable leave event', () => {
  ...
  assert.match(removalBlock, /const previousTargets = buildChatGroupBridgeUpdateTargets/, 'removed members must remain delivery targets for the removal control');
