...
function Fixture() {
  ...
  const turn: DesktopChatTurnSnapshot = { id: 'local-turn', sessionId: `${cloudGroupAgentRuntimeSessionId('owner', 'group')}:request:request`, replyToMessageId: 'request',
  ...
  const canonical = { profile: { id: 'profile', humanIdentityId: 'human:owner' },
    ...
    messages: [{ id: 'processing-slot', sessionId: 'group', senderIdentityId: 'agent:owner', senderRole: 'owned-agent', messageKind: 'agent-turn',
      ...
      content: { requestId: 'request', sourceConversationId: 'cloud-group-agent:group', senderOwnerAccountId: 'owner', deliveryState: phase === 2 ? 'queued' : phase >= 10 ? 'complete' : 'processing', ...(phase >= 10 ? { tools } : {}) } }],
  } as CanonicalSessionState;
  const projected = projectCloudGroupLiveTurns(canonical, phase >= 4 ? { request: turn } : {}, 'owner')!;
  const response = createCanonicalSessionReadModel(projected).messages('group');
  ...
  return <>
    ...
    <AppShellFrame rootThemeClass="theme-light" isNativeShell isLayoutResizing={false} windowSize={{ width: 1440, height: 1040 }} leftWorkspaceWidth={320}
      ...
      mainContent={<ChatPaneLayout hasHeader>
        ...
        <ChatSessionPane viewport={{ sessionKey: 'group', messages, scrollRef, scrollClassName: 'app-chat-pane-transcript-scroll min-h-0 flex-1 overflow-x-hidden overscroll-contain',
