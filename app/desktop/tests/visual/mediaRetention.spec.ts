...
test('offscreen media unloads without collapsing history or breaking an open preview', async ({ page }) => {
  ...
  await expect(page.locator('[data-leased-image]')).toHaveCount(0);
  await expect(page.locator('video')).toHaveCount(0);
  expect(await imageHandle!.evaluate(node => node.hasAttribute('src'))).toBe(false);
  expect(await actualImageHandle!.evaluate(node => node.hasAttribute('src'))).toBe(false);
