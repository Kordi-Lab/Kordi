...
test('filterParticipantSpaces hides unmaterialized agent shells from the Agent channel', () => {
  ...
  const agentSpaces = filterParticipantSpaces(spaces, '', 'agent');

  assert.deepEqual(agentSpaces.flatMap((space) => space.sessions), []);
});
...
test('filterParticipantSpaces keeps agent sessions after their first user-visible message', () => {
  ...
  assert.deepEqual(
    filterParticipantSpaces(spaces, '', 'agent').flatMap((space) => space.sessions.map((session) => session.id)),
    ['session:agent-ready'],
  );
});
