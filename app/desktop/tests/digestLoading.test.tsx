...
test('initial loading is not an empty result, errors offer a read-only retry, even in a hidden webview', async () => {
  ...
  try {
    ...
    assert.match(host.querySelector('.digest-status')!.textContent!, /Loading/);
    ...
    assert.match(host.querySelector('.digest-status')!.textContent!, /Couldn't reach Kordi/);
    assert.equal(host.querySelector('[aria-label="Brief"]')!.textContent, '');
    ...
    const retry = host.querySelector<HTMLButtonElement>('.digest-status button')!;
