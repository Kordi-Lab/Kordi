...
test('desktop cloud provider-auth snapshot uses provider-specific runtime defaults', () => {
  ...
  assert.match(source, /cloud_provider_auth_snapshot_model\("openai", None\)/);
  assert.match(source, /cloud_provider_auth_snapshot_model\("anthropic", None\)/);
  assert.match(source, /cloud_provider_auth_snapshot_model\("github-copilot", None\)/);
