...
test('compact model route menu uses semantic dual-theme floating surface tokens', () => {
  ...
  assert.match(css, /\.app-compact-model-menu\s*{[\s\S]*--app-compact-model-menu-bg:\s*var\(--app-surface-float\);/);
  assert.match(css, /\.app-compact-model-menu\s*{[\s\S]*--app-compact-model-menu-save-bg:\s*var\(--app-button-primary-bg\);/);
  assert.match(css, /\.bridge-app\.theme-light \.app-compact-model-menu,\n\.app-compact-model-menu-light\s*{[\s\S]*--app-compact-model-menu-bg:\s*var\(--app-surface-float\);/);
  assert.match(css, /\.app-compact-model-menu-light\s*{[\s\S]*--utility-foreground:\s*var\(--app-text-primary\);/);
  assert.match(css, /\.app-compact-model-menu-light\s*{[\s\S]*--utility-muted-text:\s*var\(--app-text-muted\);/);
});
...
test('compact model route menu sits above transcript fold controls and uses opaque dark glass', () => {
  ...
  const darkRootRule = css.match(/\.app-compact-model-menu\s*\{[\s\S]*?--app-compact-model-menu-bg:\s*var\(--app-surface-float\);[\s\S]*?\n\}/)?.[0] ?? '';
  ...
  assert.match(darkRootRule, /--app-compact-model-menu-bg:\s*var\(--app-surface-float\)/);
  assert.match(darkRootRule, /--app-divider:\s*var\(--app-border-subtle\)/);
  assert.doesNotMatch(darkRootRule, /rgba\(43, 43, 46|0 22px 48px/);
