...
impl TuiController {
    ...
    async fn handle_submission(
        ...
        match submission {
            ...
            TuiSubmission::MenuSelection { menu_id, value } => {
                if let Err(err) = self
            ...
            TuiSubmission::ApprovalDecision { .. } => Ok(()),
            TuiSubmission::CancelLocalAction => {
            ...
            TuiSubmission::EditQueuedMessages => {
    ...
    pub(crate) fn handle_approval_submission(&mut self, submission: TuiSubmission) -> Result<()> {
        let (choice, steer_message) = match submission {
            ...
            TuiSubmission::CancelLocalAction => (TuiApprovalChoice::Deny, None),
            _ => return Ok(()),
        };
        ...
        self.send_command(TuiCommand::CloseApprovalDialog);
        self.send_command(TuiCommand::SetLocalActionActive(false));
        self.send_command(TuiCommand::SetInput(String::new()));
        self.send_command(TuiCommand::SetStatusLine(match choice {
            TuiApprovalChoice::ApproveOnce => "Approved bash command".to_string(),
            TuiApprovalChoice::ApproveForSession => {
