...
impl LocalSlashCommandHost for TuiController {
    ...
    fn slash_reload(&mut self) -> Result<()> {
        self.send_command(TuiCommand::SetStatusLine(
            "Reload not yet supported in tui mode. Use /quit and restart.".to_string(),
        ));
        Ok(())
    }
    ...
    fn slash_export(&mut self, path: Option<&str>) -> Result<()> {
        ...
        match crate::tui::session::export_session(
            &self.session_setup.conn,
            &self.session_setup.session_id,
            &file_path,
        ) {
    ...
    fn slash_import(&mut self, path: Option<&str>) -> Result<()> {
        ...
        self.send_command(TuiCommand::SetStatusLine(format!(
            "Import from {path} not yet supported in tui mode."
        )));
        Ok(())
    }
