...
fn persist_tui_execution_mode(mode: ExecutionMode) -> Result<()> {
    let mut settings = Settings::load_global();
    ...
    pub(super) fn apply_setting_value(&mut self, setting_id: &str, value: &str) -> Result<()> {
        match setting_id {
            ...
            "compaction-enabled" => {
                ...
                persist_tui_compaction_settings(
