...
fn runtime_identity_remains_at_the_tail_for_every_provider() {
    ...
    let request = crate::CompletionRequest {
        system_prompt: "Stable Agent policy".into(),
        messages: history[..2].to_vec(),
        tools: vec![
    ...
    let mut after = super::build_codex_request_body(&crate::CompletionRequest {
        messages: history,
        ..request
    });
    ...
    assert_eq!(
        &next_input.as_array().unwrap()[..previous_input.as_array().unwrap().len()],
