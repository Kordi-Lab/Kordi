...
use crate::tui::format_tool_result_content;
...
fn transcript_mode_active_turn_uses_spinner_status() {
    ...
    let _ = state.apply_command(TuiCommand::TurnStart { turn_index: 0 });
    state.mode = crate::tui::types::TuiMode::Transcript;
...
fn thinking_and_writing_status_render_with_elapsed_progress() {
    ...
    let _ = state.apply_command(TuiCommand::TurnStart { turn_index: 0 });
    let _ = state.apply_command(TuiCommand::ThinkingDelta("pondering".to_string()));
    state.on_tick();
    ...
    let _ = state.apply_command(TuiCommand::TextDelta("answer".to_string()));
    state.on_tick();
...
fn tool_use_rows_stay_aligned_when_focused_in_transcript_mode() {
    let mut config = TuiAppConfig::default();
...
fn transcript_blank_line_rhythm_matches_user_tool_text_flow() {
    ...
    transcript.append_root_block(
        crate::tui::transcript::NewBlock::new(
            crate::tui::transcript::BlockKind::UserMessage,
            "prompt",
        )
...
fn summary_notes_render_with_header_block() {
    ...
    transcript.append_root_block(
        crate::tui::transcript::NewBlock::new(
            crate::tui::transcript::BlockKind::SystemNote,
            "branch summary",
        )
