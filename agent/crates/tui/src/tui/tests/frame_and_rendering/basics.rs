...
fn header_and_input_borders_follow_tui_color_theme() {
    ...
    state.color_theme = crate::tui::spinner::ColorTheme::Lavender;
    state
        .spinner
        .set_color_theme(crate::tui::spinner::ColorTheme::Lavender);
    state.prepare_for_render();
    let frame = build_frame(&state);
...
#[test]
fn local_action_status_spinner_uses_tui_color_theme() {
    let mut state = TuiState::new(
        TuiAppConfig::default(),
        Size {
            width: 80,
    ...
    state.color_theme = crate::tui::spinner::ColorTheme::Ocean;
    state
        .spinner
        .set_color_theme(crate::tui::spinner::ColorTheme::Ocean);
