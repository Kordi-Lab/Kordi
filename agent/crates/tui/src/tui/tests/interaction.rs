...
fn turn_end_returns_to_normal_mode_when_auto_following() {
    ...
    let _ = state.apply_command(TuiCommand::TurnEnd);
    assert!(!state.has_active_turn());
    assert_eq!(state.mode, TuiMode::Normal);
    assert!(
...
#[test]
fn dragging_selection_copies_character_ranges_without_exiting_tui() {
    let mut transcript = Transcript::new();
...
fn push_note_creates_visible_content_in_frame() {
    ...
    let _ = state.apply_command(TuiCommand::PushNote {
