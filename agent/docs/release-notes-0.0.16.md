...
- New command: `kordi setup browser`
  - explains what is missing
  - shows platform-specific installation guidance
  - can optionally persist `KORDI_BROWSER` shell configuration
...
- clean `kordi-cli` release builds no longer emit dead-code warnings from the new tool registry surface
...
| Full CLI test suite | `cargo +stable test -q -p kordi-cli --bins --tests` | Passed (`126 passed`) |
| Core library tests | `cargo +stable test -q -p kordi-core --lib` | Passed (`59 passed`) |
| Tools scheduler tests | `cargo +stable test -q -p kordi-tools scheduler -- --nocapture` | Passed (`2 passed`) |
| Plugin-host lifecycle serialization tests | `cargo +stable test -q -p kordi-plugin-host test_serialize_event -- --nocapture` | Passed (`4 passed`) |
| CLI tool registry tests | `cargo +stable test -q -p kordi-cli tool_registry -- --nocapture` | Passed (`4 passed`) |
| OAuth debug redaction | `cargo +stable test -q -p kordi-cli oauth_credentials_debug_redacts_tokens -- --nocapture` | Passed (`1 passed`) |
| GitHub/Copilot debug redaction | `cargo +stable test -q -p kordi-cli debug_output_redacts_github_and_copilot_tokens -- --nocapture` | Passed (`1 passed`) |
| Auth entry redaction | `cargo +stable test -q -p kordi-cli auth_entry_debug_redacts_secret_fields -- --nocapture` | Passed (`1 passed`) |
| Auth store redaction | `cargo +stable test -q -p kordi-cli auth_store_debug_lists_provider_names_without_values -- --nocapture` | Passed (`1 passed`) |
