...
- For approved daily frontend testing, use one `pnpm dev:cloud:connect` connection and `pnpm dev:cloud:shared --profile <task> --port <frontend-port>` for each preview. Preserve shared test accounts, files, and volumes; closing one preview must not stop the shared connection. Follow [shared development testing](docs/testing/shared-development.md).
- For isolated backend work, use `pnpm dev:cloud:remote` with explicit task-owned ports and a named profile. Validate the local API and OAuth callback with `pnpm doctor:dev` before sign-in. Never commit private target settings or login details, and never run resets or destructive fixtures against shared development.
